...
// Function to create a formatted display date
...
// Function to create a pending appointment card
function makePendingApptCard(appt) {
  const dateDisplayText = createDisplayDate(appt.date);

  const apptElem = `<div class="card" id="appt-${appt["appt_id"]}">
...
function makeApptCard(appt, nextAppt) {
  ...
  const apptElem = `<div class="card" id="appt-${appt["appt_id"]}">
                ...
                ${
                  nextAppt
                    ? `<h4>${nextAppt["first_name"]} ${nextAppt["last_name"]}</h4>`
                    : `<span style="display:none"></span>`
                }
                <p>Approved: yes | Completed: ${
                  !appt.completed
                    ? `<button onclick="markCompleted(${appt.appt_id})">Mark Completed</button>`
                    : "yes"
                }</p>
                <p>${appt.notes}</p>
        </div>`;
...
// Function to get pending appointments from the server
function getPendingAppointments() {
  axios
    .get("http://localhost:8765/pending")
    .then((res) => {
...
// Function to get upcoming appointments from the server
function getUpcomingAppointments() {
  axios
    .get("http://localhost:8765/upcoming")
    .then((res) => {
...
// Function to get past appointments from the server
function getPastAppointments() {
  axios
    .get("http://localhost:8765/appt")
    .then((res) => {
      for (let i = 0; i < res.data.length; i += 2) {
        const appt = res.data[i];
        const nextAppt = res.data[i + 1] || null;
...
// Function to mark an appointment as approved
function markApproved(id) {
  ...
  axios
    .put("http://localhost:8765/approve", { apptId: id })
...
function markCompleted(id) {
  ...
  axios
    .put("http://localhost:8765/complete", { apptId: id })
    .then(() => getPastAppointments())
    .catch((err) => console.log(err));
}

// Calling the functions to fetch and display appointments
